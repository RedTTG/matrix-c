...
uniform vec4 uColor;
uniform vec2 uTexSize;     // pass atlas width/height
uniform float uScale;
...
uniform vec4 uCutoff; // x:left, y:bottom, z:right, w:top
...
void main() {
    ...
    float edge =
    texture(uTexture, vUV + vec2(-px.x, 0)).a +
    texture(uTexture, vUV + vec2(px.x, 0)).a +
    texture(uTexture, vUV + vec2(0, -px.y)).a +
    texture(uTexture, vUV + vec2(0, px.y)).a;
